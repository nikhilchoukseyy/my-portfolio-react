...
const HomePage = () => {
  ...
  return (
    <div className='bg-gray-950 h-[100vh] flex flex-col items-center text-white'>
      ...
      <div className="w-[44vh] md:hidden flex justify-center items-center text-center px- py-2 fixed bottom-0 bg-gray-900 rounded-full mb-10  gap-1">
        {tabs.map((tab) => (
          <button
            key={tab.id}
            onClick={() => handleTabClick(tab.id)}
            className={`${activeTab === tab.id ? "text-blue-500" : "text-white/70"
              } relative px-1 text-sm font-medium transition hover:text-white`}
          >
            {activeTab === tab.id && (
              <motion.span
                layoutId="bubble"
